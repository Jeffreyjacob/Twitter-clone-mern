...
const ProfilePage = () => {
	...
	const { data: user, isLoading, refetch, isRefetching } = useQuery({
		...
		queryFn: async () => {
			try {
				const res = await fetch(`${API_BASE_URL}/api/v2/user/profile/${username}`, {
					method: "GET",
					credentials: "include"
				})
				const data = res.json()
	...
	const { mutate: updateProfile, isPending: isUpdatingProfile } = useMutation({
		mutationFn: async () => {
			try {
				const res = await fetch(`${API_BASE_URL}/api/v2/user/update`, {
					method: "POST",
					credentials: "include",
					headers: {
							...
							<div className='relative group/cover'>
								...
								<div className='avatar absolute -bottom-16 left-4'>
									<div className='w-32 rounded-full relative group/avatar'>
										<img src={profileImg || user?.profileImg || imagePlaceholder} />
